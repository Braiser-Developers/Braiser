...
### `extension/debug.html` 与 `extension/src/debug.ts`

可见的调试 bridge 页面。它负责持有和 `braiser-mcp` 的 WebSocket 长连接。
...
- 显示目标页面的标题和 URL。
- 打开 `debug.html`，让浏览器插件保持与 MCP 的连接。
- 手动下载目标页面运行时 DOM。
...
5. 用户从 popup 打开 `debug.html`。
6. debug 页面连接到 `ws://127.0.0.1:17832`。
7. 本地 Agent 调用 MCP tool。
8. `braiser-mcp` 将请求转发给 debug 页面。
9. debug 页面把请求转发给 background。
10. background 选择 `Braised` 组最后一个 tab，注入或复用 content script。
11. content script 读取页面、生成 agent-html 或执行 act。
12. MCP 服务清洗、保存或直接返回结果给 Agent。
